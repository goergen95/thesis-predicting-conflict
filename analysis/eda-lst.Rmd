...
```{r read-data}
files = list.files("../data/vector/extraction/", pattern = "_LST", full.names = T)
...
```{r acf-states}
acf(acf_data$states, lag.max = 24, na.action = na.pass)
```

## Basins

```{r acf-basins}
acf(acf_data$basins, lag.max = 24,, na.action = na.pass)
```
