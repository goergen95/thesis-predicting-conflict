...
```{r read-data}
files = list.files("../data/vector/extraction/", pattern = "DEP", full.names = T)
...
data %>%
  ...
  theme_classic() +
  labs(y="Count", x = "Dependency Ratio", color = "Unit of Analysis", fill = "Unit of Analysis")
...
```{r map-value}
tm_shape(plt_data) +
  tm_polygons(col = "value", palette = "-RdYlBu", breaks = c(0,50,75,100,150,200,250), border.col = "gray", lwd = .5) +
  tm_facets("unit")
```
