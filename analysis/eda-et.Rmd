...
```{r read-data}
data = readRDS("../data/vector/predictor_cube.rds")
data %<>%
  filter(grepl("^ET", var)) %>%
  as_tibble() %>%
  mutate(buffer = as.numeric(str_sub(var,4,-1)),
         var = str_sub(var,1,2),
         buffer = if_else(is.na(buffer),0, buffer)) %>%
  select(id, unit, buffer, var, time, value)
str(data)
```
...
data %>%
  ...
  theme_classic() +
  labs(y="ET [mm]", x = "Time", color = "Unit of Analysis")
...
dec_data %>%
  ...
  theme_classic() +
  labs(y = "ET", x = "Time", color = "Unit of Analysis") +
...
poly_adm$month = month(poly_adm$month, label = T)
poly_bas$month = month(poly_bas$month, label = T)
```
...
```{r map-states-value}
tm_shape(poly_adm) +
  tm_polygons("ET", palette = "RdYlBu", style = "fisher", border.col = "white", lwd = .5) +
  tm_facets("month")
```
