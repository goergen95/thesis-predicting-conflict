...
```{r read-data}
files = list.files("../data/vector/extraction/", pattern = "GDP", full.names = T)
...
data %>%
  ...
  theme_classic() +
  labs(y="GDP", x = "Time", color = "Unit of Analysis")
...
```{r map-states-value}
tm_shape(poly_adm) +
  tm_polygons(col = "value", palette = "-RdYlBu", breaks = c(0, 1000, 2000, 4000, 8000, 16000, 32000, +Inf), border.col = "white", lwd = .5) +
  tm_facets("time")
```
