# Zonal stats for variable precipitation
...
ras_files = list.files("data/raster/chirps/", full.names = T)
...
for (i in 1:length(unit_files)){
  ...
  if(length(name_split) == 3){
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_", name_split[2],"_PREC.gpkg"))
  } else {
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_PREC.gpkg"))
  }
  
  raster_cube(col, view) %>%
    zonal_statistics(aoi, expr = "mean(PREC)", out_path = filename)
