# Zonal Statistics for variable LST
...
ras_files = list.files("data/raster/lst/", full.names = T)
...
for (i in 1:length(unit_files)){
  ...
  if(length(name_split) == 3){
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_", name_split[2],"_LST.gpkg"))
  } else {
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_LST.gpkg"))
  }
  
  raster_cube(col, view) %>%
    zonal_statistics(aoi, expr = "mean(LST)", out_path = filename)
