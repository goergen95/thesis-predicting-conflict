# Zonal Statistics for variable ET
...
ras_files = list.files("data/raster/et_m/", pattern = ".tif", full.names = T)
...
for (i in 1:length(unit_files)){
  ...
  if(length(name_split) == 3){
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_", name_split[2],"_ET.gpkg"))
  } else {
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_ET.gpkg"))
  }
  
  raster_cube(col, view) %>%
    zonal_statistics(aoi, expr = "mean(ET)", out_path = filename, overwrite = TRUE)
