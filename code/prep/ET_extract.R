# Extraction of daily ET raster sets
...
for(y in year){
  ...
  for (i in 19:46){
    ...
    formula = paste0("(ET/", length(dates), ")")
    ...
    start_time = Sys.time()
    raster_cube(DB, view, mask = image_mask("ET", values = 32761:32767),
                chunking = c(8, 4000, 4000)) %>%
      select_bands("ET") %>%
      fill_time(method = "locf") %>%
      # select_time(c("2005-06-26")) %>%
      apply_pixel(formula, "ET") -> cube 
    cube %>%
      write_tif(dir = "data/raster/et/", prefix = "et_", COG = F, pack = packing,
