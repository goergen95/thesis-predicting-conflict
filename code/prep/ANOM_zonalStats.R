# Extraction of zonal statistics of precipitation anomalies
...
ras_files = list.files("data/raster/anom/", full.names = T)
...
for (i in 1:length(unit_files)){
  ...
  if(length(name_split) == 3){
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_", name_split[2],"_ANOM.gpkg"))
  } else {
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_ANOM.gpkg"))
  }
  
  raster_cube(col, view) %>%
    zonal_statistics(aoi, expr = "mean(ANOM)", out_path = filename)
