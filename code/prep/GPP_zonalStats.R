# Zonal Statistics for variable GPP
...
source("code/setup.R")
gdalcubes_options(threads = 1, debug = T, cache = T)

unit_files = list.files("data/vector/", pattern = "mask", full.names = T)
ras_files = list.files("data/raster/gpp_m/", pattern = ".tif", full.names = T)
...
for (i in 1:length(unit_files)){
  ...
  if(length(name_split) == 3){
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_", name_split[2],"_GPP.gpkg"))
  } else {
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_GPP.gpkg"))
  }
  if(file.exists(filename)){
    next
  } else {
  raster_cube(col, view, chunking = c(240, 3000, 3000)) %>%
    zonal_statistics(aoi, expr = "mean(GPP)", out_path = filename, overwrite = TRUE)
  }
}
