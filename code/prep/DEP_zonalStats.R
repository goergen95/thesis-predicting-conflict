# Extraction of zonal statistics for variable dependency ration
...
ras_files = list.files("data/raster/dep_ratio/", full.names = T)

col = create_image_collection(ras_files, date_time =seq(as.Date("2000-01-01"),
                                                        as.Date("2019-12-31"),
                                                        by = "year"),
                              band_names = c("DEP", "SUP"))
...
for (i in 1:length(unit_files)){
  ...
  if(length(name_split) == 3){
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_", name_split[2],"_DEP.gpkg"))
  } else {
    filename = file.path("data/vector/extraction", paste0(name_split[1],"_DEP.gpkg"))
  }
  
  raster_cube(col, view, chunking = c(240, 1000, 1000)) %>%
    fill_time("locf") %>%
    zonal_statistics(aoi, expr = c("sum(DEP)", "sum(SUP)"), out_path = filename)
}
